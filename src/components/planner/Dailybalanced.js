...
import { balancedDietPlan } from '../../models/DailybalancedDiet';


function getRandomUser(balancedDietPlan) {
 // Get all the keys (user names) from the users object
 let userNames = Object.keys(balancedDietPlan);
 ...
 return balancedDietPlan[randomUserName];
}
...
const Dailybalanced = ({navigation}) => {
 ...
 const [user, setUser] = useState(getRandomUser(balancedDietPlan));
 ...
 setUser(getRandomUser(balancedDietPlan));
 };
