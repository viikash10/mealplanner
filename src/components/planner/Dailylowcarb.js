...
import { lowCarbDietPlan } from '../../models/Dailylowcarb' ;


function getRandomUser(lowCarbDietPlan) {
 // Get all the keys (user names) from the users object
 let userNames = Object.keys(lowCarbDietPlan);
 ...
 return lowCarbDietPlan[randomUserName];
}
...
const Dailybalanced = ({navigation}) => {
 ...
 const [user, setUser] = useState(getRandomUser(lowCarbDietPlan));
 ...
 setUser(getRandomUser(lowCarbDietPlan));
 };
